...
public class VideoInfoParseManager {
    ...
    public void parseVideoInfo(VideoCacheInfo cacheInfo, Map<String, String> headers,
                               Map<String, Object> extraParams,boolean needSign, IVideoInfoParsedListener listener) {
        mListener = listener;
        mHeaders = headers;
        //这两个值开发者可以设置
        ...
        VideoProxyThreadUtils.submitRunnableTask(() -> {
            if (ProxyCacheUtils.getConfig().useOkHttp()) {
                parseVideoInfoByOkHttp(cacheInfo,needSign);
            } else {
                parseVideoInfo(cacheInfo,needSign);
            }
        });
    }

    //使用okhttp网路框架去请求数据
    private void parseVideoInfoByOkHttp(VideoCacheInfo cacheInfo,boolean needSign) {
        if (TextUtils.equals(VideoParams.UNKNOWN, mContentType)) {
            ...
            if (videoUrl.contains("m3u8")) {
                ...
                parseNetworkM3U8Info(cacheInfo,needSign);
            } else {
    ...
    private void parseVideoInfo(VideoCacheInfo cacheInfo,boolean needSign) {
        if (TextUtils.equals(VideoParams.UNKNOWN, mContentType)) {
            String videoUrl = cacheInfo.getVideoUrl();
            if (videoUrl.contains("m3u8")) {
                //这种情况下也基本可以认为video是M3U8类型，虽然判断不太严谨，但是mediaplayer也是这么做的
     ...
     * @param cacheInfo
     */
    private void parseNetworkM3U8Info(VideoCacheInfo cacheInfo,boolean needSign) {
        try {
            M3U8 m3u8 = M3U8Utils.parseNetworkM3U8Info(cacheInfo.getVideoUrl(), cacheInfo.getVideoUrl(), mHeaders, 0,needSign);
    ...
    public void parseProxyM3U8Info(VideoCacheInfo cacheInfo, Map<String, String> headers,boolean needSign, VideoInfoParsedListener listener) {
        mHeaders = headers;
        mListener = listener;
        File proxyM3U8File = new File(cacheInfo.getSavePath(), cacheInfo.getMd5() + StorageUtils.PROXY_M3U8_SUFFIX);
