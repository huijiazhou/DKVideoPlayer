...
public class VideoProxyCacheManager {
     ...
     */
    public void startRequestVideoInfo(String videoUrl,boolean needSign) {
        startRequestVideoInfo(videoUrl, new HashMap<>(),needSign);
    }
     ...
     */
    public void startRequestVideoInfo(String videoUrl, Map<String, String> headers,boolean needSign) {
        startRequestVideoInfo(videoUrl, headers, new HashMap<>(),needSign);
    }
     ...
     *                    详情见VideoParams
     */
    public void startRequestVideoInfo(String videoUrl, Map<String, String> headers, Map<String, Object> extraParams,boolean needSign) {
        ...
        if (videoCacheInfo == null) {
            ...
            VideoInfoParseManager.getInstance().parseVideoInfo(videoCacheInfo, headers, extraParams, needSign,new IVideoInfoParsedListener() {
                @Override
            ...
            if (videoCacheInfo.getVideoType() == VideoType.M3U8_TYPE) {
                //说明视频类型是M3U8类型
                final Object lock = VideoLockManager.getInstance().getLock(md5);
                VideoInfoParseManager.getInstance().parseProxyM3U8Info(videoCacheInfo, headers,needSign, new VideoInfoParsedListener() {
                    @Override
                    public void onM3U8ParsedFinished(M3U8 m3u8, VideoCacheInfo cacheInfo) {
                        notifyLocalProxyLock(lock);
                        mM3U8LocalProxyMd5Set.add(md5);
